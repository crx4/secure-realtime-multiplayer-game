...
suite('Unit Tests', () => {
  ...
  suite('Collectible class', () => {
    test('Collectible class generates a collectible item object.', done => {
      const testItem = new Collectible(100, 100, '', Date.now());
  ...
  suite('Player class', () => {
    test('Player class generates a player object.', done => {
      const testPlayer = new Player(100, 100, 0, Date.now());
    ...
    test("calculateRank(arr) returns the player's rank string.", done => {
      const testPlayer1 = new Player(100, 100, 0, 1);
      const testPlayer2 = new Player(150, 150, 0, 2);
      testPlayer1.score = 5;
      testPlayer2.score = 3;
      const testArr = [ testPlayer1, testPlayer2 ];

      // Account for possible space
      assert.match(testPlayer1.calculateRank(testArr), /1?\/?2/);
      assert.match(testPlayer2.calculateRank(testArr), /2\s?\/\s?2/);
      done();
    });
  });
